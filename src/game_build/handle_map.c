...
static int  flood_fill(t_parser *parser, int **wall, int y, int x)
{
	...
	if (wall[y][x] == 1 || parser->map1[y][x] == 1)
		return (0);
	wall[y][x] = 1;
	if (y == 0 || y == parser->map_height - 1 || x == 0 || x == parser->map_width - 1)
	{
		parser->invalid_map = true;
		printf("Map validation error: Found gap in wall at position [%d,%d]\n", y, x);
...
static void check_walls(t_parser *parser)
{
	...
	else
	{
		...
		while (y < parser->map_height)
		{
			...
			while (x < parser->map_width)
			{
				if (parser->map1[y][x] == 0) // Walkable space
				{
